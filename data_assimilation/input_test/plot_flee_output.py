...
def plot_camps(data: pd.DataFrame, config: str, output: str) -> None:
    ...
    for i in range(len(cols)):
        ...
        # Split sim and data from the column name,
        ...
        plt.xlabel("Hours elapsed", fontsize=14)
        ...
        (label2,) = plt.plot(data_filtered.index, y2, "b", linewidth=5, label="{} expected".format(name[0]))

        plt.legend(handles=[label1, label2], loc=0, prop={"size": 14})

        plt.savefig("{}/{}.pdf".format(output, name[0]), bbox_inches = 'tight')
...
def plot_numagents(data: pd.DataFrame, config: str, output: str) -> None:
    ...
    if "refugee_debt" in data.columns:
        ...
        plt.legend(
            labels=["Total Refugees", "Refugees in Shelters", "Expected", "Refugee Debt"],
        )

    else:
        data.loc[
            :,
            ["total refugees (simulation)", "refugees in camps (Expected)", "raw Expected refugee count"],
        ].plot(linewidth=5)
...
def plot_flee_output(input_dir: str, output_dir: str, mscale: bool = False) -> None:
    ...
    if isfile(whole_df_PATH):
        ...
        whole_df = pd.read_csv(whole_df_PATH, sep=",", encoding="utf-16", index_col="Day")
