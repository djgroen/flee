...
def AddCSVTables(table1, table2):
    ...
    for c2 in table2:
        ...
        while c2[0] > table1[offset][0]:
            table = np.vstack(
                [table, [table1[offset][0], last_c2[1] + table1[offset][1]]])
...
def ConvertCsvFileToNumPyTable(csv_name, data_type="int", date_column=0, count_column=1,
...
class DataTable:
    ...
    def __init__(
        ...
        data_layout="data_layout_refugee.csv",
        start_date="2012-02-29",
        csvformat="generic"
    ):
        """
    ...
    def get_daily_difference(self, day, day_column=0, count_column=1, Debug=False,
        ...
        when enabled, the function can return negative numbers when the new total is higher than
        the older one.
        """
        ...
        if self.override_refugee_input is True:
            ref_table = self.data_table[
                self._find_headerindex("total (modified input)")]

        # Refugees only come in *after* day 0.
        if int(day) == 0:
            ref_table = self.data_table[0]  # noqa: F841
    ...
    def get_interpolated_data(self, column, day):
        ...
        for i in range(1, len(ref_table)):
            ...
            if day < ref_table[i, self.days_column]:
                ...
                fraction = float(day - old_day) / \
                    float(ref_table[i, self.days_column] - old_day)

                if fraction > 1.0:
                    print("Error with days_column: ",
    ...
    def get_raw_data(self, column, day):
        ...
        for i in range(0, len(ref_table)):
            if day >= ref_table[i][self.days_column]:
                ...
                old_day = ref_table[i][self.days_column]  # noqa: F841
            else:
                break
        return int(old_val)
