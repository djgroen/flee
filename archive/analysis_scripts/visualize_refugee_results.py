...
def plot_decision_analysis(scenario_dir, scenario_name, output_file):
    ...
    # 3. Population by location type (over time)
    ax = axes[1, 0]
    pop_by_location = metrics.get('population_by_location', {})
    locations = results.get('locations', [])
    
    if pop_by_location and locations and timesteps:
        # Create a mapping from location name to type
        loc_to_type = {loc['name']: loc.get('type', 'unknown') for loc in locations}
        
        # Calculate population by type over time
        type_data = defaultdict(list)
        for t_idx, t in enumerate(timesteps):
            type_pops = {'conflict': 0, 'town': 0, 'camp': 0}
            for loc_name, pops in pop_by_location.items():
                if t_idx < len(pops):
                    loc_type = loc_to_type.get(loc_name, 'unknown')
                    if loc_type in type_pops:
                        type_pops[loc_type] += pops[t_idx]
            for loc_type, pop in type_pops.items():
                type_data[loc_type].append(pop)
        
        # Plot time series for each type
        colors = {'conflict': '#c0392b', 'town': '#95a5a6', 'camp': '#2ecc71'}
        for loc_type in ['conflict', 'town', 'camp']:
            if loc_type in type_data and type_data[loc_type]:
                ax.plot(timesteps, type_data[loc_type], 
        ...
        ax.set_title('Population by Location Type Over Time')
        ax.legend()
        ax.grid(True, alpha=0.3)
    else:
...
def create_all_visualizations(results_dir="refugee_simulation_results", latest_only=True):
    """Create all visualizations for all scenarios.
    
    Args:
        results_dir: Directory containing simulation results
        latest_only: If True (default), only process the latest run for each scenario type
    """
    results_dir = Path(results_dir)
    ...
    all_scenario_dirs = [d for d in results_dir.iterdir() if d.is_dir() and d.name.startswith('run_')]
    
    if not all_scenario_dirs:
        print(f"❌ No scenario directories found in {results_dir}")
        return
    
    # If latest_only, keep only the most recent run for each scenario type
    if latest_only:
        import glob
        import os
        ...
        for scenario_type in scenario_types:
            ...
            if runs:
                latest_runs[scenario_type] = Path(runs[0]).name
        
        # Filter to only latest runs
        scenario_dirs = [d for d in all_scenario_dirs if d.name in latest_runs.values()]
        print(f"📊 Processing latest runs only: {len(scenario_dirs)} scenarios")
        for scenario_type, run_name in latest_runs.items():
            print(f"   - {scenario_type}: {run_name}")
    else:
        scenario_dirs = all_scenario_dirs
        print(f"📊 Processing all runs: {len(scenario_dirs)} scenarios")
