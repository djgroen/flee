...
import sys
import time
from dflee.DisasterSimulationSettings import DisasterSimulationSettings
...
if __name__ == "__main__":
  ...
  if len(sys.argv)<4:
    print("Please run using: python3 run.py <your_csv_directory> <your_refugee_data_directory> <duration in days or days> <optional: simsettings.yml> > <output_directory>/<output_csv_filename>")
  ...
  if len(sys.argv)==5:
    dflee.DisasterSimulationSettings.ReadFromYML(sys.argv[4])
  else:
    dflee.DisasterSimulationSettings.ReadFromYML("simsettings.yml")

  dflee.DisasterSimulationSettings.FloodInputFile = "%s/floods.csv" % input_csv_directory

  e = dflee.Ecosystem()

  ig = dInputGeography.InputGeography()

  ig.ReadFloodInputCSV(dflee.DisasterSimulationSettings.FloodInputFile)
  ...
  d = handle_refugee_data.RefugeeTable(csvformat="generic", data_directory=validation_data_directory, start_date=start_date, data_layout="data_layout.csv", population_scaledown_factor=DisasterSimulationSettings.optimisations["PopulationScaleDownFactor"])
