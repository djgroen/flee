...
def calculateLinkWeight(
  ...
  if link.endpoint.marker is False:
    ...
    weight = float(getEndPointScore(agent=agent, link=link)
          / (float(SimulationSettings.move_rules["Softening"] + link.get_distance() + prior_distance)**SimulationSettings.move_rules["DistancePower"]) * getCapMultiplier(link.endpoint, numOnLink=int(link.numAgents))
