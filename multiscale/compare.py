...
def compare(input_dir_1, input_dir_2, first_sim, second_sim,
            mscale_model, **kwargs):
    ...
    refugee_data1 = pd.read_csv(
        "{}/out.csv".format(input_dir_1),
        sep=",", encoding="latin1", index_col="Day"
    )
    refugee_data2 = pd.read_csv(
        "{}/out.csv".format(input_dir_2),
        sep=",", encoding="latin1", index_col="Day"
    )
    if input_dir_3[-5:] == "whole":
        refugee_data3 = pd.read_csv(
            "{}/out.csv".format(input_dir_3),
            sep=",", encoding="latin1", index_col="Day"
        )
    ...
    if "refugee_debt" in refugee_data1.columns:
        ...
        refugees_in_camps1, = plt.plot(
            ...
            y_data,
            linewidth=5,
            label="Refugees in camps (simulation) {} Coupling".format(
                first_sim)
        )
        ...
        if uncoupled_model == "whole":
            ...
            total_refugees3, = plt.plot(
                ...
                y_data,
                linewidth=5,
                label="Total refugees (simulation) {} Coupling".format(
                    uncoupled_model)
            )
            ...
            refugees_in_camps3, = plt.plot(
                ...
                y_data,
                linewidth=5,
                label="Refugees in camps (simulation)- {}".format(
                    uncoupled_model)
            )

        if uncoupled_model == "whole":
            plt.legend(
                handles=[total_refugees1, total_refugees2, total_refugees3,
                         refugees_in_camps1, refugees_in_camps2,
                         refugees_in_camps3, raw_refugee_count,
            ...
            plt.legend(
                handles=[total_refugees1, total_refugees2, refugees_in_camps1,
                         refugees_in_camps2, raw_refugee_count,
                         refugee_debt_simulation],
                loc=0,
                prop={"size": 14}
            )
    ...
    diffdata1 = sim_errors1.abs_diff(
        rescaled=False) / np.maximum(un_refs1, np.ones(len(un_refs1)))
    diffdata2 = sim_errors2.abs_diff(
        rescaled=False) / np.maximum(un_refs2, np.ones(len(un_refs2)))
    if uncoupled_model == "whole":
        diffdata3 = sim_errors3.abs_diff(
            rescaled=False) / np.maximum(un_refs3, np.ones(len(un_refs3)))

    diffdata1_rescaled = sim_errors1.abs_diff() / \
        np.maximum(un_refs1, np.ones(len(un_refs1)))

    diffdata2_rescaled = sim_errors2.abs_diff() / \
        np.maximum(un_refs2, np.ones(len(un_refs2)))

    if uncoupled_model == "whole":
        diffdata3_rescaled = sim_errors3.abs_diff() / \
            np.maximum(un_refs3, np.ones(len(un_refs3)))

    print(first_sim, ": Averaged error normal: ", np.mean(diffdata1),
          ", rescaled: ", np.mean(diffdata1_rescaled),
          ", len: ", len(diffdata1)
          )
    print(second_sim, ": Averaged error normal: ", np.mean(diffdata2),
          ", rescaled: ", np.mean(diffdata2_rescaled),
          ", len: ", len(diffdata2)
          )

    if uncoupled_model == "whole":
        print(uncoupled_model, ": Averaged error normal: ", np.mean(diffdata3),
              ", rescaled: ", np.mean(diffdata3_rescaled),
              ", len: ", len(diffdata3)
              )
    ...
    if uncoupled_model == "whole":
        plt.legend(
            handles=[labeldiff_rescaled1, labeldiff_rescaled2,
                     labeldiff_rescaled3, labeldiff1, labeldiff2, labeldiff3],
            loc=1,
            prop={"size": 14}
        )
    ...
    for i in range(len(cols2)):
        ...
        if uncoupled_model != 'whole':
            ...
            plt.legend(
                handles=[label1, label2, label3, label4],
                loc=0,
                prop={"size": 14}
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--second_sim",
                        required=True, action="store", type=str,
                        choices=["file", "muscle3", "weather/file",
                                 "weather/muscle3"]
                        )
    ...
    parser.add_argument("--uncoupled_model",
                        ...
                        default=uncoupled_model
                        )

    args, unknown = parser.parse_known_args()
    print(args)

    input_dir_1 = os.path.join(
        work_dir, "out", args.first_sim, args.mscale_model
    )
    input_dir_2 = os.path.join(
        work_dir, "out", args.second_sim, args.mscale_model
    )
    input_dir_3 = os.path.join(
        work_dir, "out", args.uncoupled_model
    )
    output_dir = os.path.join(
        work_dir, "out", args.mscale_model + "--comparison"
    )
    ...
    compare(input_dir_1, input_dir_2,
            args.first_sim, args.second_sim,
            args.mscale_model,
