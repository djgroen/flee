...
def create_refugee_animation(scenario_dir, scenario_name, output_file, fps=3):
    ...
    colors = {
        'S1': '#E69F00',  # Orange for System 1 (reactive/flocking) - colorblind-friendly
        'S2': '#56B4E9',  # Blue for System 2 (deliberative/individual) - colorblind-friendly
    ...
    for loc in locations:
        ...
        if loc_type == 'camp' or 'SafeZone' in name or 'Border' in name:
            ax.scatter(x, y, s=1200, c='#56B4E9', marker='s',  # Blue square, larger - colorblind-friendly
                      alpha=0.8, zorder=3, edgecolors='#0072B2', linewidths=5)
        ...
        # Conflict zone - DISTINCT (match nuclear style)
            ...
            ax.scatter(x, y, s=1200, c='#E69F00', marker='*',  # Orange star, larger - colorblind-friendly
                      alpha=0.8, zorder=3, edgecolors='#D55E00', linewidths=5)
            ax.text(x, y + 30, 'CONFLICT ZONE', ha='center', fontsize=16, fontweight='bold',
                   bbox=dict(boxstyle='round,pad=0.5', facecolor='#FFE6CC', 
                            edgecolor='#D55E00', linewidth=2, alpha=0.9))
...
def create_all_animations(results_dir="refugee_simulation_results", latest_only=True):
