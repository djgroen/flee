...
# python3 compare.py --first_sim (file or muscle3) --second_sim (file or
# muscle3) --mscale_model (micro or macro) --uncoupled_model name4
# (optional)
...
def compare(input_dir_1, input_dir_2, first_sim, second_sim, mscale_model, **kwargs):
    ...
    refugee_data1 = pd.read_csv(
        "{}/out.csv".format(input_dir_1), sep=",", encoding="latin1", index_col="Day"
    )
    refugee_data2 = pd.read_csv(
        "{}/out.csv".format(input_dir_2), sep=",", encoding="latin1", index_col="Day"
    )
    if input_dir_3[-5:] == "whole":
        refugee_data3 = pd.read_csv(
            "{}/out.csv".format(input_dir_3), sep=",", encoding="latin1", index_col="Day"
        )
    ...
    """
    refugees_with_debt = [
        refugee_data1.loc[:, ["total refugees (simulation)",
                              "refugees in camps (simulation)"]
                          ] +
        refugee_data2.loc[:, ["total refugees (simulation)",
                              "refugees in camps (simulation)",
    ...
    if "refugee_debt" in refugee_data1.columns:
        ...
        (total_refugees1,) = plt.plot(
            ...
            y_data,
            linewidth=5,
            label="Total refugees (simulation) {} Coupling".format(first_sim),
        )
        ...
        (refugees_in_camps1,) = plt.plot(
            ...
            y_data,
            linewidth=5,
            label="Refugees in camps (simulation) {} Coupling".format(first_sim),
        )
        ...
        (total_refugees2,) = plt.plot(
            ...
            y_data,
            linewidth=5,
            label="Total refugees (simulation) {} Coupling".format(second_sim),
        )
        ...
        (refugees_in_camps2,) = plt.plot(
            ...
            y_data,
            linewidth=5,
            label="Refugees in camps (simulation) {} Coupling".format(second_sim),
        )
        ...
        if uncoupled_model == "whole":
            ...
            (total_refugees3,) = plt.plot(
                ...
                y_data,
                linewidth=5,
                label="Total refugees (simulation) {} Coupling".format(uncoupled_model),
            )
            ...
            (refugees_in_camps3,) = plt.plot(
                ...
                y_data,
                linewidth=5,
                label="Refugees in camps (simulation)- {}".format(uncoupled_model),
            )
    ...
    un_refs1 = refugee_data1.loc[:, ["refugees in camps (UNHCR)"]].to_numpy().flatten()
    un_refs2 = refugee_data1.loc[:, ["refugees in camps (UNHCR)"]].to_numpy().flatten()
    un_refs3 = refugee_data1.loc[:, ["refugees in camps (UNHCR)"]].to_numpy().flatten()
    ...
    for i in location_names:
        loc_errors1.append(
            pfo.plotme(input_dir_1, refugee_data1, i, legend_loc=4, naieve_model=nmodel)
        )
        loc_errors2.append(
            pfo.plotme(input_dir_2, refugee_data2, i, legend_loc=4, naieve_model=nmodel)
        )
        if uncoupled_model == "whole":
            loc_errors3.append(
                pfo.plotme(input_dir_3, refugee_data3, i, legend_loc=4, naieve_model=nmodel)
            )

    sim_errors1 = pfo.SimulationErrors(location_errors=loc_errors1)
    sim_errors2 = pfo.SimulationErrors(location_errors=loc_errors2)
    sim_errors3 = pfo.SimulationErrors(location_errors=loc_errors3)
    ...
    if uncoupled_model == "whole":
        diffdata3 = sim_errors3.abs_diff(rescaled=False) / np.maximum(
            un_refs3, np.ones(len(un_refs3))
        )
    ...
    (labeldiff_rescaled2,) = plt.plot(
        ...
        diffdata2_rescaled,
        linewidth=5,
        label="Error {} Coupling".format(second_sim),
    )
    ...
    if uncoupled_model == "whole":
        (labeldiff_rescaled3,) = plt.plot(
            ...
            diffdata3_rescaled,
            linewidth=5,
            label="Error {} Simulation".format(uncoupled_model),
        )

    if uncoupled_model == "whole":
        plt.legend(
            handles=[labeldiff_rescaled1, labeldiff_rescaled2, labeldiff_rescaled3],
            loc=7,
            prop={"size": 14},
        )
    else:
        plt.legend(
            handles=[labeldiff_rescaled1, labeldiff_rescaled2],
            loc=7,
            prop={"size": 14},
        )
    ...
    (labeldiff2,) = plt.plot(
        ...
        diffdata2,
        linewidth=5,
        label="error- {} Coupling (not rescaled)".format(second_sim),
    )
    ...
    if uncoupled_model == "whole":
        (labeldiff3,) = plt.plot(
            ...
            diffdata3,
            linewidth=5,
            label="error- {} Simulation (not rescaled)".format(uncoupled_model),
        )
    ...
    if uncoupled_model == "whole":
        ...
        plt.legend(
            handles=[labeldiff_rescaled1, labeldiff_rescaled2, labeldiff1, labeldiff2],
            loc=1,
            prop={"size": 14},
        )
    ...
    for i in range(len(cols2)):
        ...
        (label2,) = plt.plot(refugee_data1.index, y2, "b", linewidth=5, label="UNHCR data")
        ...
        plt.savefig(os.path.join(output_dir, "{}.png".format(name1[0])))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--first_sim", required=True, action="store", type=str, choices=["file", "muscle3"]
    )
    parser.add_argument(
        "--second_sim",
        required=True,
        action="store",
        type=str,
        choices=["file", "muscle3", "weather/file", "weather/muscle3"],
    )
    parser.add_argument(
        "--mscale_model", required=True, action="store", type=str, choices=["micro", "macro"]
    )
    ...
    compare(
        ...
        args.second_sim,
        args.mscale_model,
        input_dir_3=input_dir_3,
        uncoupled_model=args.uncoupled_model,
    )

    # print(
    #     "To find about the results of comparison between "
    #     "{} and {} ({} model), please see the {} directory.".format(
    #         sys.argv[1], sys.argv[2], sys.argv[3], output_dir)
    # )
