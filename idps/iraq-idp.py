...
if __name__ == "__main__":
    ...
    ig.ReadLocationsFromCSV("iraq/iraq-locations.csv", columns=[
                            "region", "area (km^2)", "pop/cap", "name", "capital pop.", "gps_x", "gps_y"])
    ...
    d_spawn = handle_refugee_data.RefugeeTable(
        csvformat="generic", data_directory="iraq/spawn_data", start_date="2014-05-01")
    d = handle_refugee_data.RefugeeTable(
        csvformat="generic", data_directory="iraq/idp_counts", start_date="2014-05-01")
    ...
    """
        ...
        for l in e.locations:
            new_IDPs = int(d_spawn.get_field(l.name, t + 1) -
                           d_spawn.get_field(l.name, t))
            if new_IDPs > 0:
                l.movechance = 1.0
            else:
                l.movechance = 0.001
        ...
        for i in range(0, len(camp_names)):
            ...
            camp_pops += [d.get_field(camp_names[i],
                                      t, FullInterpolation=True)]
        ...
        for i in range(0, len(locations)):
            output += ",%s,%s,%s" % (locations[i].numAgents,
                                     loc_data[i], errors[i])
        ...
        if float(sum(loc_data)) > 0:
            ...
            output += ",%s,%s,%s,%s,%s,%s" % (float(np.sum(abs_errors)) / float(refugees_raw + 1), int(
