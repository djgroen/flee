...
def identifiability_report(idx_df, df):
    ...
    if not kappa_sensitive:
        lines.append("  DIAGNOSIS: With the perception layer removed (Day 5), κ now operates")
        lines.append("  on native location.conflict values via the precomputed potential field.")
        lines.append("  If κ remains insensitive, the conflict gradient (c_here - c_best) is")
        lines.append("  uniform across the design space at the current network topology.")
        lines.append("  ACTION: Re-run Day 4 sweep after Day 5 wiring is complete.")
        all_pass = False
