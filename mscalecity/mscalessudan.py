...
import sys
from flee.postprocessing import analyze_graph
...
def date_to_sim_days(date):
    return DataTable.subtract_dates(date, "2016-06-01")


if __name__ == "__main__":

    end_time = 426
    last_physical_day = 426
    submodel_id = 0
    ...
    c.setCouplingFilenames("micro", "macro")
    if(submodel_id > 0):
        c.setCouplingFilenames("macro", "micro")

    ig = InputGeography.InputGeography()

    ig.ReadLocationsFromCSV("~/mscalessudan/locations-%s.csv" % submodel_id)

    ig.ReadLinksFromCSV("~/mscalessudan/routes-%s.csv" % submodel_id)

    ig.ReadClosuresFromCSV("~/mscalessudan/closures-%s.csv" % submodel_id)
    ...
    coupled_locations = ["Kapoeta_East", "Pochalla",
                         "Terekeka", "Bor", "Pariang", "Panyikang"]
