...
class Link(flee.Link):
  ...
  def __init__(self, endpoint, distance, link_type="drive", forced_redirection=False):
    super().__init__(endpoint, distance, link_type, forced_redirection)
...
class Ecosystem(flee.Ecosystem):
  ...
  def add_conflict_zone(self, name, change_movechance=True):
    super().add_conflict_zone(name, change_movechance)
    """
    Adds a conflict zone. Default weight is equal to population of the location.
    """
    self.change_movechance = change_movechance
  ...
  def linkUp(self, endpoint1, endpoint2, distance="1.0", forced_redirection=False, link_type=None):
    super().linkUp(endpoint1, endpoint2, distance, forced_redirection, link_type)
