...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if (sys.argv[1]).isnumeric():
            ...
            end_time = 820
            last_physical_day = 820
            duration = flee.SimulationSettings.ReadFromCSV(sys.argv[
                                                                              1])
            if duration > 0:
                end_time = duration
    ...
    ig.ReadLocationsFromCSV("conflict_input/car_input_csv/locations.csv")

    ig.ReadLinksFromCSV("conflict_input/car_input_csv/routes.csv")

    ig.ReadClosuresFromCSV("conflict_input/car_input_csv/closures.csv")
    ...
    d = handle_refugee_data.RefugeeTable(
        csvformat="generic", data_directory="conflict_validation/car2014/", start_date="2013-12-01", data_layout="data_layout.csv")
    ...
    # To our knowledge, all level 2 registration procedures were put in place
    # by the end of 2016.
    ...
    # also "2014-05-11" and "2015-06-02"
    lm["Moyo"].capacity *= d.correctLevel1Registrations("Moyo", "2015-06-02")
    ...
    # lm["Moyo"].capacity = d.getMaxFromData("Moyo", last_physical_day ) #
    # blip in the data set, set capacity manually.
    ...
    # lm["Bossobolo"].capacity = d.getMaxFromData("Bossobolo",
    # last_physical_day) #camp excluded
    ...
    lm["Brazaville"].capacity = d.getMaxFromData(
        "Brazaville", last_physical_day)

    output_header_string = "Day,"

    camp_locations = ["Amboko", "Belom", "Dosseye", "Gondje", "Moyo", "East",
                      "Adamaoua", "Mole", "Bili", "Boyabu", "Mboti", "Inke", "Betou", "Brazaville"]
