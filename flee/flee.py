...
class Person:
    ...
    """
        ...
        return float(link.endpoint.scores[awareness_level] / float(
            SimulationSettings.SimulationSettings.Softening + link.distance))
    ...
    def selectRoute(self):
        ...
        for i in range(0, linklen):
            ...
            # forced redirection: if this is true for a link, return its value immediately.
                ...
                weights[i] = self.getLinkWeight(self.location.links[i],
                                                SimulationSettings.SimulationSettings.AwarenessLevel)
...
class Location:
    def __init__(self, name, x=0.0, y=0.0, movechance=0.001, capacity=-1, pop=0, foreign=False, country="unknown"):
        ...
        if isinstance(movechance, str):
            if "camp" in movechance.lower():
                ...
                print("Error in creating Location() object: cannot parse movechance value of ", movechance,
                      " for location object with name ", name, ".", file=sys.stderr)
    ...
    def print(self):
        print(
            "Location name: %s, X: %s, Y: %s, movechance: %s, cap: %s, pop: %s, country: %s, conflict? %s, camp? %s" % (
            self.name, self.x, self.y, self.movechance, self.capacity, self.pop, self.country, self.conflict,
            self.camp), file=sys.stderr)
        for l in self.links:
            print("Link from %s to %s, dist: %s, pop. %s" % (self.name, l.endpoint.name, l.distance, l.numAgents),
                  file=sys.stderr)
    ...
    """
        ...
        residual = self.numAgents - (
                    nearly_full_occ * cap_limit)  # should be a number equal in range [0 to 0.1*self.numAgents].
...
class Ecosystem:
        ...
        if len(self.closures) > 0:
            for c in self.closures:
                if time == c[3]:
                    if c[0] == "country":
                        if Debug:
                            print("Time = %s. Closing Border between [%s] and [%s]" % (time, c[1], c[2]),
                                  file=sys.stderr)
                ...
                if time == c[4]:
                    if c[0] == "country":
                        if Debug:
                            print("Time = %s. Reopening Border between [%s] and [%s]" % (time, c[1], c[2]),
                                  file=sys.stderr)
    ...
    """
        ...
        for i in range(0, len(self.locations[x].links)):
            if self.locations[x].links[i].endpoint.name is not endpoint:
                ...
                self.locations[x].closed_links += [
                    copy.copy(self.locations[x].links[i])]  # we copy the route link to have a backup.
        ...
        if not removed:
            print("Warning: cannot remove link from %s, as there is no link to %s" % (startpoint, endpoint),
                  file=sys.stderr)
        return removed
    ...
    """
        ...
        if not reopened:
            print("Warning: cannot reopen link from %s, as there is no link to %s" % (startpoint, endpoint),
                  file=sys.stderr)
        return reopened
    ...
    """
        ...
        print("%s location 1 way [%s] in direction %s (%s)." % (mode, location_name, direction, dir_mode),
              file=sys.stderr)
                    ...
                    if mode == "close":
                        ...
                        if dir_mode > 0:
                            if self.close_link(self.locationNames[i], link_set[j].endpoint.name, twoway=False):
                                link_set = self.locations[
                                    i].links  # shrink the link list. # This operation affects the overall loop, so no major operations should take place after this.
                        else:
                            j += 1
                        ...
                        if dir_mode > 0:
                            if self.reopen_link(self.locationNames[i], link_set[j].endpoint.name, twoway=False):
                                link_set = self.locations[
                                    i].closed_links  # shrink the closed link list. # This operation affects the overall loop, so no major operations should take place after this.
                            else:
                                j += 1
    ...
    """
            ...
            if self.locations[i].country == source_country:
                ...
                while j < len(link_set):
                    if link_set[j].endpoint.country == dest_country:
                        if Debug:
                            print("starting to %s border 1 way [%s/%s] [%s/%s]" % (
                            mode, source_country, self.locations[i].name, dest_country, link_set[j].endpoint.name),
                                  file=sys.stderr)
                        changed_anything = True
                        if mode == "close":
                            if self.close_link(self.locationNames[i], link_set[j].endpoint.name, twoway=False):
        ...
        if not changed_anything:
            print("Warning: no link closed when closing borders between %s and %s." % (source_country, dest_country),
                  file=sys.stderr)
            ...
            if self.locationNames[i] == name:
                if name not in self.conflict_zone_names:
                    ...
                    if SimulationSettings.SimulationSettings.InitLogLevel > 0:
                        ...
                        print("New total pop. in conflict zones: ", self.conflict_pop, file=sys.stderr)
                    return

        print("Diagnostic: self.locationNames: ", self.locationNames, file=sys.stderr)
        print(
            "ERROR in flee.add_conflict_zone: location with name [%s] appears not to exist in the FLEE ecosystem (see diagnostic above)." % (
        ...
        if SimulationSettings.SimulationSettings.TakeRefugeesFromPopulation:
            if location.conflict:
                if location.pop > 0:
                    ...
                    print(
                        "ERROR: Number of agents in the simulation is larger than the combined population of the conflict zones. Please amend locations.csv.", file=sys.stderr)
                    location.print()
    ...
    """
        ...
        for i in range(0, len(self.agents)):
            if self.agents[i].location.name not in location_names:
                ...
                self.agents[
                    i].location.numAgents -= 1  # agent is removed from exosystem and number of agents in location drops by one.
    ...
    """
        ...
        self.locations[endpoint1_index].links.append(
            Link(self.locations[endpoint1_index], self.locations[endpoint2_index], distance, forced_redirection))
