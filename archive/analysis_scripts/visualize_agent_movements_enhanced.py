#!/usr/bin/env python3
"""
Enhanced Agent Movement Visualization

Shows:
- Clear network topology with edges
- Large, visible individual agents
- S1 vs S2 behavioral differences (flocking vs individual)
- Topology-specific layouts
"""
...
def create_enhanced_animation(results_dir, topology_name, output_file, fps=2):
    """Create enhanced animation with visible agents and network structure."""
    
    print(f"\n🎬 Creating enhanced animation for {topology_name} topology...")
    ...
    if not locations:
        print(f"⚠️  No topology structure found, using agent data locations")
        unique_locs = agent_df[['current_location', 'x', 'y']].drop_duplicates(subset='current_location')
        locations = [{'name': row['current_location'], 'x': row['x'], 'y': row['y']} 
    ...
    for route in routes:
        ...
        if from_loc and to_loc:
            ax.plot([from_loc['x'], to_loc['x']], 
                   [from_loc['y'], to_loc['y']], 
                   'k-', linewidth=1, alpha=0.2, zorder=1)
    ...
    for loc in locations:
        ...
        if 'SafeZone' in name:
            ax.scatter(x, y, s=500, c=colors['SafeZone'], marker='s', 
                      alpha=0.4, zorder=2, edgecolors='black', linewidths=3)
            ax.text(x, y + 20, 'SAFE', ha='center', fontsize=11, fontweight='bold')
        elif 'Facility' in name:
            ax.scatter(x, y, s=500, c=colors['Facility'], marker='*', 
                      alpha=0.4, zorder=2, edgecolors='black', linewidths=3)
            ax.text(x, y + 20, 'FACILITY', ha='center', fontsize=11, fontweight='bold')
        else:
            # Color by conflict level
            alpha = 0.3 + (1 - conflict) * 0.2
            ax.scatter(x, y, s=200, c=colors['Location'], marker='o', 
                      alpha=alpha, zorder=2, edgecolors='black', linewidths=1)
    
    # Initialize agent plots (MUCH LARGER)
    scatter_s1 = ax.scatter([], [], c=colors['S1'], s=150, alpha=0.8, 
                           label='System 1 (Reactive/Flocking)', zorder=10, 
                           edgecolors='darkred', linewidths=2, marker='o')
    scatter_s2 = ax.scatter([], [], c=colors['S2'], s=150, alpha=0.8, 
                           label='System 2 (Deliberative/Individual)', zorder=10, 
                           edgecolors='darkgreen', linewidths=2, marker='s')
    
    # Title
    title = ax.text(0.5, 0.97, '', transform=ax.transAxes, 
                   ha='center', fontsize=18, fontweight='bold')
    subtitle = ax.text(0.5, 0.93, '', transform=ax.transAxes, 
    ...
    # Track agent positions for S1 flocking visualization
    ...
    def animate(frame):
        ...
        # Determine S1 vs S2 based on location (proxy for now)
        # S1: high-conflict areas (Facility, Ring1) - reactive/flocking
        # S2: safer areas - deliberative/individual
        ...
        if len(s1_agents) > 0:
            ...
            scatter_s1.set_sizes([150] * len(s1_agents))  # Large, visible
        else:
            scatter_s1.set_offsets(np.empty((0, 2)))
        
        if len(s2_agents) > 0:
            scatter_s2.set_offsets(np.c_[s2_agents['x'], s2_agents['y']])
            scatter_s2.set_sizes([150] * len(s2_agents))  # Large, visible
        else:
        ...
        subtitle.set_text(f'S2 Activation: {s2_rate:.1f}% | Agents: {len(t_data)} | S1: {len(s1_agents)} | S2: {len(s2_agents)}')
...
def create_all_enhanced_animations(results_dir=None):
    """Create enhanced animations for all topologies."""
    ...
    print("\n" + "="*70)
    print("CREATING ENHANCED AGENT MOVEMENT ANIMATIONS")
    print("="*70)
    
    for topology in topologies:
        output_file = results_dir / f'{topology.lower()}_agent_movements_enhanced.mp4'
        create_enhanced_animation(results_dir, topology, output_file, fps=2)
    
    print("\n✅ Enhanced animation creation complete!")
...
if __name__ == "__main__":
    ...
    create_all_enhanced_animations(results_dir)
