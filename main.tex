...
%==============================================================================
% SLIDE 11b: Phase 1 Duration (Section 5.2)
%==============================================================================
\begin{frame}{Phase 1 Duration (Section 5.2)}

\paragraph{Phase 1 duration.} The dimensionless Phase 1 duration
$\tau^* = t^* \cdot v / d^*(\beta)$ normalizes the phase timing by
network geometry and move speed. Here $t^*$ is the step at which
mean $P_{S2}$ reaches its minimum (Phase 1 end), $v$ is
\texttt{max\_move\_speed} (km/step), and $d^*(\beta)$ is the network
distance (km) from the NPP to the location where conflict intensity
$c = \ln(2)/\beta$ (the $\Omega = 0.5$ threshold). Calibration target:
$\tau^* \approx 1$ (median agent reaches the $\Omega = 0.5$ zone at
Phase 1 end).

\vspace{0.5em}
\textbf{Prediction 3 (updated):} $t^* > 0$ and $\tau^* \leq 1$.
Phase 1 minimum occurs after crisis onset; dimensionless duration
$\tau^* \leq 1$ ensures the phase transition matches spatial scale
(not an artifact of a too-short simulation window).
...
$d^*(\beta)$ & $d^*$ & $(0, \infty)$ & Derived & Network distance to $\Omega=0.5$ threshold (km) \\
$\tau^*$ & $\tau^*$ & $(0, \infty)$ & Derived & Dimensionless Phase 1 duration: $t^* \cdot v / d^*$ \\
\bottomrule
\end{tabular}
...
timestep. We use $v = 20$~km/step (10~km/h), the lower bound of this
range, consistent with documented gridlock on Route~6. It does not
enter the count of free cognitive parameters ($\alpha$, $\beta$, $\kappa$).

\end{frame}

%==============================================================================
% SLIDE 12b: Parameter Reference (Section 8)
%==============================================================================
\begin{frame}{Parameter Reference (Section 8)}
...
\midrule
$d^*(\beta)$ & Network distance (km) from NPP to location with $c = \ln(2)/\beta$ & Characteristic spatial scale ($\Omega = 0.5$ threshold) \\
$\tau^*$ & $t^* \cdot v / d^*(\beta)$ & Dimensionless Phase 1 duration; target $\approx 1$ \\
\bottomrule
\end{tabular}
\caption{Derived parameters for phase structure validation.}
\end{table}
...
%==============================================================================
% SLIDE 12c: Source Verification Notes (Section 11)
%==============================================================================
\begin{frame}{Source Verification Notes}

\paragraph{Fukushima calibration.} The physical parameters
\texttt{conflict\_movechance} and \texttt{max\_move\_speed} are fixed from
empirical data before cognitive parameter calibration.
\texttt{conflict\_movechance} = 0.25 per 2-hour timestep is derived from the
observed inner-zone ($<5$~km) clearing time in \citet{Hayano2013}:
approximately 95\% departure within 18~hours implies
$p_{\text{step}} = 1 - 0.05^{1/9} \approx 0.25$.
\texttt{max\_move\_speed} = 20~km per 2-hour timestep (10~km/h) is the
lower bound of the congested vehicle speed range reported in
\citet{Shimazaki2012}.
...
\bibitem[Hayano and Adachi, 2013]{Hayano2013}
Hayano, R. and Adachi, R. (2013).
\newblock Estimation of the total population moving into and out of the
20~km evacuation zone during the Fukushima NPP accident.
