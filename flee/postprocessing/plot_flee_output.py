...
def plotme(out_dir, data, name, offset=0, legend_loc=4, naieve_model=True):
    ...
    if offset == 0:
        labelsim, = plt.plot(days, y1, linewidth=8,
                             label="%s simulation" % (name.title()))
    if offset > 0:
        labelsim, = plt.plot(
            days[:-offset], y1[offset:], linewidth=8, label="%s simulation" % (name.title()))

    # Plotting line representing UNHCR data.
    labeldata, = plt.plot(days, y2, 'o-', linewidth=8,
                          label="%s UNHCR data" % (name.title()))

    # Add label for the naieve model if it is enabled.
    plt.legend(handles=[labelsim, labeldata],
    ...
    labelsim, = plt.plot(days, y1_rescaled, linewidth=8,
                         label="%s simulation" % (name.title()))

    labeldata, = plt.plot(days, y2, linewidth=8,
                          label="%s UNHCR data" % (name.title()))
    ...
    if naieve_model:
        ...
        labelnaieve, = plt.plot(days, n1, linewidth=6,
                                label="%s naieve model" % (name.title()))
        labelnaieve, = plt.plot(days, n2, linewidth=6,
    ...
    if naieve_model:
        plt.legend(handles=[labelsim, labelnaieve, labeldata],
                   loc=legend_loc, prop={'size': 18})
    else:
        plt.legend(handles=[labelsim, labeldata],
                   loc=legend_loc, prop={'size': 18})
  ...
  """
    ...
    if naieve_model:
        ...
        print("%s,%s,%s,%s,%s,%s,%s,%s,%s" % (out_dir, name, lerr.errors["MASE7"], lerr.errors["MASE7-sloped"], lerr.errors[
              "MASE7-ratio"], lerr.errors["MASE30"], lerr.errors["MASE30-sloped"], lerr.errors["MASE30-ratio"], lerr.errors["N"]))
...
def plotme_minimal(out_dir, data, name):
    ...
    d = handle_refugee_data.DataTable(
        "mali2012/refugees.csv", csvformat="mali-portal")
...
def plot_flee_output(in_dir, out_dir):
  ...
  """
    ...
    if nmodel:
        print("%s & %s & %s & %s & %s & %s & %s\\\\" % (out_dir, sim_errors.get_error("MASE7"), sim_errors.get_error("MASE7-sloped"),
                                                        sim_errors.get_error("MASE7-ratio"), sim_errors.get_error("MASE30"), sim_errors.get_error("MASE30-sloped"), sim_errors.get_error("MASE30-ratio")))
    ...
    diffdata_rescaled = sim_errors.abs_diff() / np.maximum(un_refs,
                                                           np.ones(len(un_refs)))
    print(out_dir, ": Averaged error normal: ", np.mean(diffdata),
