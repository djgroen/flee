...
# this function finds the IPC food data and assigns it to two variables, critict and IPC_all. critict is the dates column. IPC_all treats the dates column as the index.
def initiate_food():
...
class Ecosystem(flee.Ecosystem):
  ...
  def update_IPC_MoveSpeed(self,line_IPC,IPC_all):
    self.IPC_locations = []
    for i in range(0,len(self.locationNames)):
      if self.locations[i].country=="South_Sudan":

        #first, we need to update the IPC scores for each location in the flee simulation.
        self.locations[i].IPC=IPC_all.loc[line_IPC,self.locations[i].region]

        #next, give each location a movement speed based on the locations IPC value?
      if self.IPCAffectsMovementSpeed:
        #self.locations[i].movespeed=SimulationSettings.SimulationSettings.MaxMoveSpeed*(self.locations[i].IPC/100.0)
  ...
  def update_IPC_MC(self,line_IPC,IPC_all):                        #maybe better (less computation time)
    ...
    for i in range(0,len(self.locationNames)):
      if self.locations[i].country=="South_Sudan":                       #needed??
		...
		#4. Adjust move chances, based on hypothesis 5: "the greater the food insecurity, the less likely migrants are to move" defualt move chance, multiplied by 1-percentage of people with severe food insecurity
        if self.IPCAffectsMoveChance:
          if not self.locations[i].conflict and not self.locations[i].camp and not self.locations[i].forward:
            self.locations[i].movechance=SimulationSettings.SimulationSettings.DefaultMoveChance*(1-self.locations[i].IPC/100.0)
