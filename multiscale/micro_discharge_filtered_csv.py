...
locations = pd.read_csv("{}/locations-1.csv".format(input_dir))
...
if __name__ == "__main__":
    ...
    discharge = pd.read_csv(
        "{}/ssudan_dis_filtered.csv".format(data_dir),
        sep=",",
        encoding="latin1"
    )
    ...
    discharge.to_csv(
        "{}/river_discharge.csv".format(data_dir),
        na_rep="",
        float_format="%.3f",
        index=False
    )

    print("The ({}) river_discharge.csv is stored in {}".format(
        args.input_dir, data_dir)
    )
