...
def updateLocationScore(time: int, loc) -> None:
    ...
    if loc.foreign is True:
        ...
        try:
            score *= 1.0 - (loc.attributes["region_IPC_level"] / 100.0)**1.5
        except KeyError:
            print(f"ERROR: attribute region_IPC_level missing for location {loc.name} in region {loc.region}.", file=sys.stderr)
            sys.exit()
