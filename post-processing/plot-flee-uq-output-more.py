...
def plotme(out_dir, data, name, offset=0, legend_loc=3,
    ...
    if naieve_model:
        fig.savefig("%s/camp-%s-%s-rescaled-N_V2.png" %
                    (out_dir, name, legend_loc))
    else:
        fig.savefig("%s/camp-%s-%s-rescaled_V2.png" %
                    (out_dir, name, legend_loc))
...
def calculate_error(out_dir, data, name, offset=0):
...
if __name__ == "__main__":
    ...
    for filename in csv_file_address:
        ...
        refugee_data = pd.read_csv(filename, index_col='Day',
                                   sep=',', encoding='latin1')
        ...
        '''
        print(dir, ": Averaged error normal: ", np.mean(diffdata),
              ", rescaled: ", np.mean(diffdata_rescaled), ", len: ", len(diffdata))
        '''

        labeldiff2, = ax1.plot(np.arange(len(diffdata_rescaled)),
                               diffdata_rescaled, linewidth=2, label=dir + " error")
        set_margins()

        labeldiff, = ax2.plot(np.arange(len(diffdata)), diffdata,
                              linewidth=2, label=dir + " error (not rescaled)")
    ...
    handles_set = []
    labelsim, = plt.plot(np.arange(days_number), y_error,
                         linewidth=3, label="Average Error (not rescaled)")
    handles_set.append(labelsim)

    labelsim, = plt.plot(np.arange(days_number), y_error_max,
                         linewidth=1, label="Max Error (not rescaled)",
                         color="black", linestyle='dashed')
    handles_set.append(labelsim)

    labelsim, = plt.plot(np.arange(days_number), y_error_min,
                         linewidth=1, label="Min Error (not rescaled)",
                         color="black", linestyle='dashed')
    handles_set.append(labelsim)
    ...
    labelsim, = plt.plot(np.arange(days_number), y_error_rescaled,
                         linewidth=3, label="Average Error")
    handles_set.append(labelsim)

    labelsim, = plt.plot(np.arange(days_number), y_error_rescaled_max,
                         linewidth=1, label="Max Error",
                         color="blue", linestyle='dashed')
    handles_set.append(labelsim)

    labelsim, = plt.plot(np.arange(days_number), y_error_rescaled_min,
                         linewidth=1, label="Min Error",
                         color="black", linestyle='dashed')
    handles_set.append(labelsim)
